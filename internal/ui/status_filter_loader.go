...
func (loader *filteredStatusLoader) load(regions []string) Statuses {
	...
	for _, region := range regions {
		...
		for i := range mergedGroups {
			appendStatuses(regionalLoader.loadMergedPrefilter(region, &mergedGroups[i]))
		}
...
func (loader *filteredStatusLoader) loadPrefiltered(region string, groups filter.Groups, awsFilters []ssm.ParameterFilter) Statuses {
	...
	metas, err := loader.client.ListParameterMetadataWithFilters(loader.context(), awsFilters)
	if err != nil {
