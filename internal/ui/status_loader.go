...
func newStatusLoader(ctx context.Context, client ssm.Client, includeValues bool, progress LoadProgress, batch StatusBatch) statusLoader {
	return statusLoader{ctx: ctx, client: client, includeValues: includeValues, progress: progress, batch: batch}
}
...
func (loader statusLoader) loadByItemRegion(items inventory.Items) Statuses {
	...
	for start := 0; start < len(items); start += 10 {
		...
		for region, regionItems := range byRegion {
			...
			described := regionalClient.DescribeMany(loader.ctx, paths)
			for path := range described {
				metas[itemKey(region, path)] = described[path]
			}
			chunkValues, chunkErrs := regionalClient.GetMany(loader.ctx, paths)
...
func (loader statusLoader) loadRegions(items inventory.Items, regions []string) Statuses {
	...
	for index, region := range regions {
		if !previousCompleted.IsZero() {
			logger.LogAttrs(loader.ctx, slog.LevelDebug, "region scan gap", slog.String("region", region), slog.Int("index", index), slog.Int64("duration_ms", elapsedStatusMillis(previousCompleted)))
		}
		started := time.Now()
		logger.LogAttrs(loader.ctx, slog.LevelDebug, "region scan started", slog.String("region", region), slog.Int("index", index), slog.Int("item_count", len(items)), slog.Bool("include_values", loader.includeValues))
		regionStatuses, regionFound := loader.withClient(loader.client.ForRegion(region)).loadOneRegion(items, region)
		completed := time.Now()
		logger.LogAttrs(loader.ctx, slog.LevelDebug, "region scan completed", slog.String("region", region), slog.Int("index", index), slog.Int("status_count", len(regionStatuses)), slog.Int("found_count", len(regionFound)), slog.Int64("duration_ms", int64(completed.Sub(started)/time.Millisecond)))
...
func (loader statusLoader) loadOneRegion(items inventory.Items, region string) (statuses Statuses, found map[string]bool) {
	...
	logger.LogAttrs(loader.ctx, slog.LevelDebug, "region metadata scan started", slog.String("region", region))
	metas, err := loader.client.ListParameterMetadata(loader.ctx)
