...
func (resolver *OptionsResolver) forRecord(record *textio.Record, cloud *ssm.Metadata, exists bool) (ssm.PutParameterOptions, error) {
	...
	if fields.Allows(textio.FieldDataType) && record.HasField(textio.FieldDataType) && strings.TrimSpace(record.DataType) != "" {
	...
	if fields.Allows(textio.FieldDescription) && record.HasField(textio.FieldDescription) && strings.TrimSpace(record.Description) != "" {
